...
def place_component(floor_id: int, product_offer_id: int, lat: float, long: float, auth=Depends(JWTBearer()),
                    gis_service: GISService = Depends()):
    token = auth.credentials
    payload = get_payload(token)
    return gis_service.add_component(floor_id=floor_id, product_offer_id=product_offer_id, lat=lat, long=long)


@gis_router.delete('/delete_place_component', summary='Удалить размещение')
def delete_place_component(id: int, auth=Depends(JWTBearer()), gis_service: GISService = Depends()):
    token = auth.credentials
    payload = get_payload(token)
    return gis_service.delete_component(id=id)
